...
export default function App() {
  ...
  const exportToCSV = () => {
    const headers = ["Code", "User", "Machine", "Type", "Created At", "Expires At", "Status", "Note"];
  ...
  return (
    <div className="min-h-screen bg-[#E4E3E0] text-[#141414] font-sans p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Table Header */}
        <div className="grid grid-cols-[1fr_120px_120px_120px_120px_100px_80px] gap-4 px-4 py-2 border-y border-[#141414] font-serif italic text-[11px] opacity-50 uppercase tracking-wider">
          <div>Mã Key</div>
          <div>Người dùng</div>
          <div>Máy (HWID)</div>
          <div>Thời hạn</div>
            ...
            <AnimatePresence initial={false}>
              {filteredKeys.map((key) => (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, x: -20 }}
                  className="grid grid-cols-[1fr_120px_120px_120px_120px_100px_80px] gap-4 px-4 py-4 border-b border-[#141414]/10 hover:bg-[#141414] hover:text-[#E4E3E0] transition-all group cursor-default"
                >
                  ...
                  <div className="text-xs font-mono truncate flex items-center opacity-70 group-hover:opacity-100">
                    {editingId === key.id ? (
                      <input 
                        autoFocus
                        type="text"
                        className="bg-white text-[#141414] px-1 w-full outline-none"
                        value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                        onBlur={() => updateKey(key.id, { username: editValue })}
                        onKeyDown={(e) => e.key === 'Enter' && updateKey(key.id, { username: editValue })}
                      />
                    ) : (
                      <span 
                  ...
                  <div className="text-[10px] font-mono truncate flex items-center opacity-50 group-hover:opacity-100" title={key.hwid}>
                    {key.hwid ? key.hwid.substring(0, 8) + "..." : "CHƯA CÓ"}
                  </div>
